  ...
  */
  ...
  public void printObjectivesToFile(String path, long estimatedTime, int size){
    try {
      ...
      if (!dir.exists())
    	  dir.mkdir();
      PrintWriter writer = new PrintWriter(dir.getAbsolutePath()+"/result.txt", "UTF-8");
  ...
  //  	  writer.println(aSolutionsList_.operation());
    	  ...
    	  writer.println("------------------------------------------------------------------------------");
    	  writer.println();
    	  String str2=s.getpath()+"/resultformodeltransformation"+NSGAII.indexmodeltransformation+"/solution"+"/finalresult"+aSolutionsList_.getindex()+".atl";
