...
public abstract class AbstractTypeModificationMutator extends AbstractMutator {
	...
	protected <ToModify extends LocatedElement> List<Object> genericTypeModification(EMFModel atlModel,
		 ...
		 * anatlyzer.atlext.OCL.OclType returnop=null; List<OperationCallExp> op =
		 * (List<OperationCallExp>) wrapper.allObjectsOf(OperationCallExp.class); if (
		 * op.get(29).eContainingFeature() == ATLPackage.Literals.BINDING__VALUE ) {
		 * 
		 * Binding b = (Binding) op.get(29).eContainer(); returnop=(OclType)
		 * b.getLeftType(); } System.out.println(returnop);
		 * 
		 * Operation operation = OCLFactory.eINSTANCE.createOperation();
		 * operation.setName("zahra" ); operation.setReturnType( (returnop) ); for
		 * (OclExpression argument : op.get(29).getArguments()) { Parameter parameter =
		 * OCLFactory.eINSTANCE.createParameter(); parameter.setType ( (OclType)
		 * (argument.getInferredType()) ); //
		 * parameter.setVarName(getNiceParameterName(argument,
		 * operation.getParameters())); operation.getParameters().add(parameter); }
		 * 
		 * OclFeatureDefinition def = OCLFactory.eINSTANCE.createOclFeatureDefinition();
		 * def.setFeature (operation); StaticHelper newhelper =
		 * ATLFactory.eINSTANCE.createStaticHelper(); newhelper.setDefinition(def);
		...
		if (modifiable.size() > 0) {
			...
			/*
			 * String helper_name = helper.get(4).getDefinition().getFeature() instanceof
			 * Operation? ((Operation)helper.get(4).getDefinition().getFeature()).getName():
			 * ((Attribute)helper.get(4).getDefinition().getFeature()).getName(); String
			 * helper_return = helper.get(4).getDefinition().getFeature() instanceof
			 * Operation? toString(((Operation)helper.get(4).getDefinition().getFeature()).
			 * getReturnType()):
			 * toString(((Attribute)helper.get(4).getDefinition().getFeature()).getType());
			 * String helper_parameter = helper.get(4).getDefinition().getFeature()
			 * instanceof Operation?
			 * (((Operation)helper.get(4).getDefinition().getFeature()).getParameters()).
			 * toString():
			...
			while (checkmutationapply == false) {
				...
				if (randomInt == -2)
					...
					ReturnResult = OperationPreviousGeneration(randomInt, solution, atlModel, metamodels, modifiable,
							wrapper, featureName, comments, ReturnResult, metamodels1, metamodels2);
					checkmutationapply = true;
					...
					if (modifiable.size() > 0 && NSGAII.errorrule.contains(indexrule) && check == true) {
						...
						if (featureDefinition != null && featureDefinition.getUpperBound() == 1) {
							...
							while (findreplace == false) {
								...
								if (Operations.statecase.equals("case6")) {
									...
									List<VariableExp> variables = (List<VariableExp>) wrapper
											.allObjectsOf(VariableExp.class);
									...
									if (NSGAII.inpatternhasfilter.get(indexrule) == 1) {
										...
										if (!toString(oldFeatureValue).equals(toString(replacements.get(randomInt2)))) {
											if (!NSGAII.inpatternstringlocation
													.contains(toString(replacements.get(randomInt2))))
												findreplace = true;
											else {
												int indexof = NSGAII.inpatternstringlocation
										...
										if (!toString(oldFeatureValue).equals(toString(replacements.get(randomInt2)))
												&& !NSGAII.inpatternstringlocation
														.contains(toString(replacements.get(randomInt2))))
											findreplace = true;
							...
							comments.add("\n-- MUTATION \"" + this.getDescription() + "\" from "
									...
									+ toString(modifiable.get(randomInt)) + ":" + toString(replacements.get(randomInt2))
							...
							/*
							 * for(int p2=0;p2<NSGAII.errorrule.size();p2++) { int occu=0;
							 * 
							 * for(int p=0;p<filterrule.size();p++)
							 * 
							 * if(NSGAII.errorrule.get(p2)==filterrule.get(p)) occu=occu+1; if(occu>0)
							 * occurancerrule.add(occu);
							 * 
							 * } int countoccuranc=0; for(int p2=0;p2<occurancerrule.size();p2++)
							 * if(occurancerrule.get(p2)>=5) countoccuranc=countoccuranc+1;
							 * if(countoccuranc==occurancerrule.size()) checkmutationapply = true;
							 */
							// System.out.println(occurancerrule);
							/*
							 * for(int p=0;p<NSGAII.errorrule.size();p++) {
							 * 
							 * if(!filterrule.contains(NSGAII.errorrule.get(p))) filterrulecheck=true;
							 * 
							 * } if(filterrulecheck==false) checkmutationapply = true;
							 */
							/*
							 * boolean countusing=false; if(indexrule<NSGAII.preconditionlocation.size() &&
							 * Operations.statecase.equals("case7") ) { if(NSGAII.preconditionlocation.get(
							 * indexrule ) > Integer.parseInt(array[0])) { countusing=true; } }
							 */
								...
								if (value.size() > 0) {
									...
									if (oldFeatureValue2.eClass().getEPackage().getEClassifier("OclModelElement")
										...
										while (findreplace == false) {
											...
											if (!toString(oldFeatureValue2)
													.equals(toString(replacements.get(randomInt4))))
												findreplace = true;
										}
			...
			if (Operations.statecase.equals("case1") || Operations.statecase.equals("case7")
				...
				case 1:
					if (MyProblem.oldoperation1 != -1) {
						...
						/*
						 * if (count == 7) { MyProblem.oldoperation1 = -1; MyProblem.replaceoperation1 =
						 * -1; MyProblem.secondoldoperation1 = -1; Random number_generator = new
