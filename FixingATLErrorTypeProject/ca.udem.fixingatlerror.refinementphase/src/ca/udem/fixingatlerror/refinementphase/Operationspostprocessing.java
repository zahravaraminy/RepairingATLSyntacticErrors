...
import modification.feature.operator.refinementphase.BindingModificationMutatorpost;
import modification.feature.operator.refinementphase.NavigationModificationMutatorpost;
import modification.invocation.refinementphase.PredefinedOperationModificationMutatorpost;
import modification.type.operator.refinementphase.ArgumentModificationMutatorpost;
import modification.type.operator.refinementphase.CollectionModificationMutatorpost;
import modification.type.operator.refinementphase.InElementModificationMutatorpost;
...
public class Operationspostprocessing extends BaseTest {
	...
	public void Testeroperate(String trafo, String temporalFolder) throws ATLCoreException, transException {
		...
		System.out.println(MMRootPath2);
	...
	public List<Object> executeOperations(int num, CoSolutionpostprocessing s2, int sizeoperation, Solution solution,
			int sumfirstfit, int sumsecondfit, int sumthirdfit, FileWriter csvWriter) {
		...
		try {
			...
			case 1:
				...
				AbstractMutator[] operators = {
						...
						// new OutElementModificationMutatorpost(),
				...
				for (AbstractMutator operator : operators) {
					...
					comments = operator.generateMutants(atlModel, this.iMetaModel, this.oMetaModel, this.folderMutants,
							this.wrapper, solution);
					// Tester.wrapper2=new
					// anatlyzer.evaluation.mutators.ATLModel(this.atlModel.getResource());
			...
			case 3:
				...
				System.out.println("metamodel");
				System.out.println(this.oMetaModel.getName());
		...
		try {
			if (sizeoperation == 20) {
				...
				this.save(atlModel, NSGAIIpostprocessing.BestSolutionLocation);

				// List<Object> CoSolutionco = CalculateNumberError((EMFModel)
				// comments.get(1),(ATLModel) comments.get(0));
				// List<Object> CoSolutionco =
				// CalculateNumberError(this.atlModel,this.wrapper,sumfirstfit,sumsecondfit,sumthirdfit,csvWriter);
				/*
				 * if(comments==null) { List<Object> comments2 = new ArrayList<Object>();
				 * comments2.add((int) CoSolutionco.get(2)); comments2.add((int)
				 * CoSolutionco.get(3)); comments2.add((int) CoSolutionco.get(4)); return
				 * comments2; } else { comments.add((int) CoSolutionco.get(2));
				 * comments.add((int) CoSolutionco.get(3)); comments.add((int)
				 * CoSolutionco.get(4)); return comments; }
				 * 
				 */
			 ...
			 * this.save(atlModel, s.getnewoutputresult()+NSGAII.indexmodeltransformation);
			 * List<Object>
			 * CoSolutionco=CalculateNumberError(this.atlModel,this.wrapper,sumfirstfit,
			 * sumsecondfit,sumthirdfit,csvWriter); if(comments==null) { List<Object>
			 * comments2 = new ArrayList<Object>(); comments2.add((int)
			 * CoSolutionco.get(2)); comments2.add((int) CoSolutionco.get(3));
			 * comments2.add((int) CoSolutionco.get(4)); return comments2; } else {
			 * comments.add((int) CoSolutionco.get(2)); comments.add((int)
			 * CoSolutionco.get(3)); comments.add((int) CoSolutionco.get(4)); return
			 * comments; }
			 * 
			 * 
			 * }
	...
	private List<Object> CalculateNumberError(EMFModel atlModel2, ATLModel atlModel4, int sumfirstfit, int sumsecondfit,
		...
		try {
			...
			String newfile = setting.getnewoutputresult() + NSGAIIpostprocessing.indexmodeltransformation
					+ "/finalresult" + main.totalnumber + ".atl";
		...
		NSGAIIpostprocessing.fitness2 = nberrors;
		ReturnResult.add(nberrors);
		if (nberrors == 0 && NSGAIIpostprocessing.fixedgeneration == -1) {
			if (NSGAIIpostprocessing.counter % this.populationsize == 0)
				NSGAIIpostprocessing.fixedgeneration = NSGAIIpostprocessing.counter / (this.populationsize);
			else
				NSGAIIpostprocessing.fixedgeneration = (NSGAIIpostprocessing.counter / (this.populationsize)) + 1;
		}
		int fit3 = cocou.Coverage(atlModel2);
		NSGAIIpostprocessing.fitness3 = fit3;
		cocou.setThirdfit(fit3);
		ReturnResult.add(fit3);

		if (NSGAIIpostprocessing.counter % this.populationsize == 0) {

			float sumfirst = ((float) (sumfirstfit + NSGAIIpostprocessing.numop)) / (float) (this.populationsizedouble);
			...
			sumfirstfit = sumfirstfit + NSGAIIpostprocessing.numop;
	...
	private void printtofile(float sumfirstfit, float sumsecondfit, float sumthirdfit, FileWriter csvWriter) {
		...
		try {
			csvWriter.append(NSGAIIpostprocessing.counter / (this.populationsize) + ";");
			csvWriter.append(sumfirstfit + ";");
			csvWriter.append(sumsecondfit + ";");
			csvWriter.append(sumthirdfit + ";");
			if (NSGAIIpostprocessing.counter == this.populationsize)
				csvWriter.append("\n");
