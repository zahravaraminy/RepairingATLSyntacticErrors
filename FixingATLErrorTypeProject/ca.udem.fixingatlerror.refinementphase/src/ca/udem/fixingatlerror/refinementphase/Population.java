...
public class Population {
	...
	public ArrayList<CoSolution> random_selection() throws Exception {
		 ...
		 * this.crossover(parents.get(i),parents.get(i+1)); parents.set(i,
		 * this.mutation(parents.get(i))); parents.set(i+1,
		 * this.mutation(parents.get(i+1))); }
	...
	public void normalize_metrics() {
		...
		/*
		 * a + (x-A)*(b-a)/(B-A) where: A minimum of dataset B maximum of dataset a is
		 * from where you would like normalised data set to start b is where you would
		 * like normalised data set to end x is the value you are trying to normalise
		 * 
		...
		for (int i = 0; i < solutions.size(); i++) {
			for (int j = 0; j < this.solutions.get(0).objectives.size(); j++) {
				this.solutions.get(i).objectives.set(j,
						(double) (this.solutions.get(i).objectives.get(j) - min_objectives[j])
								/ (max_objectives[j] - min_objectives[j]));
			}
		}
		...
		for (int i = 0; i < solutions.size(); i++) {
			...
			for (int j = 0; j < this.solutions.get(0).objectives.size(); j++) {
				temporary_sum += (Math.pow((solutions.get(i).objectives.get(j) - ref.objectives[j])
						/ (max_objectives[j] - min_objectives[j]), 2));
			}
		...
		for (int i = 1; i < solutions.size(); i++) {
			if (solutions.get(i).distance < max_distance)
				min_distance = solutions.get(i).distance;
			if (solutions.get(i).distance > min_distance)
				max_distance = solutions.get(i).distance;
		}
	}
	...
	// to be used on tournament selection when checking which is the best solution
	...
	public int compare_dominance(CoSolution solution1, CoSolution solution2) {
		...
		if (dominate1 == dominate2) {
			return r_dominates(solution1, solution2, min_distance, max_distance, s); // No one dominate the other so we
																						// send r_dominance value
		}
		if (dominate1 == 1) {
	...
	public void crowdingDistanceAssignment(ArrayList<CoSolution> one_front) throws Exception {
		...
		/*
		 * for (int i = 0; i < size; i++){ front.add(one_front.get(i)); }
		 */
	...
	// it is called to select randomly the best candidate solutions to be parents
	// for the upcoming offsprings
	public ArrayList<CoSolution> tournament_selection2() throws Exception {
		...
		for (int i = 0; i < tournament_bound; i++) {
			...
			} while ((first_individual_index == second_individual_index) || (first_individual_index < 0)
	...
	void export_configuration(int pareto_size) {
		...
		try {
			...
			writer.append(
					"Average number of fronts : " + Double.toString(this.selection.average_front_number()) + "\n\n");
			writer.append("Selected solutions number (optional) : " + Double.toString(this.selection.selection.size())
					+ "\n\n");
			writer.append(
					"Selected solutions are the set of best solution (minimal distance) in each iteration (duplicated solutions removed) \n\n");
			// generate whatever data you want
