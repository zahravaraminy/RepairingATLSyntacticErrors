...
public class NSGAII extends Algorithm {
	 ...
	 * 
	 * int populationSize; int maxEvaluations; int evaluations; Setting s=new
	 * Setting(); File dir=new
	 * File(s.getpath()+"/resultformodeltransformation"+NSGAII.
	 * indexmodeltransformation); if (!dir.exists()) dir.mkdir(); QualityIndicator
	 * indicators; // QualityIndicator object int requiredEvaluations; // Use in the
	 * example of use of the // indicators object (see below) try { writer = new
	 * PrintWriter(dir.getAbsolutePath()+"/file-operatin.txt", "UTF-8"); try {
	 * this.csvWriterpareto= new
	 * FileWriter(dir.getAbsolutePath()+"/paretosize.csv");
	 ...
	 * Auto-generated catch block e.printStackTrace(); } SolutionSet population;
	 * SolutionSet offspringPopulation; SolutionSet union;
	 * 
	 * Operator mutationOperator; Operator crossoverOperator; Operator
	 * selectionOperator;
	 * 
	 * Distance distance = new Distance();
	 * 
	 * //Read the parameters populationSize = ((Integer)
	 * getInputParameter("populationSize")).intValue(); maxEvaluations = ((Integer)
	 * getInputParameter("maxEvaluations")).intValue(); indicators =
	 * (QualityIndicator) getInputParameter("indicators");
	 * 
	 * //Initialize the variables population = new SolutionSet(populationSize);
	 * evaluations = 0;
	 * 
	 * requiredEvaluations = 0;
	 * 
	 * //Read the operators mutationOperator = operators_.get("mutation");
	 * crossoverOperator = operators_.get("crossover"); selectionOperator =
	 * operators_.get("selection");
	 * 
	 * // Create the initial solutionSet
	 * 
	 * Solution newSolution; CoSolution cocou=new CoSolution(); FindFaultyRule();
	 * deleteDirectory
	 * (s.getpath()+"/resultformodeltransformation"+NSGAII.indexmodeltransformation+
	 * "/solution", false); CoSolution.check=0; // cocou.w.getSMClick(
	 * "examples/class2rel/trafo/Class2TableExperience1.atl",null);
	 * 
	 * // List<String> firstfoot=cocou.footprintscalcul(""); boolean
	 * checkiteration=false; // for (int i = 0; i < populationSize; i++) { for (int
	 * i = 0; i < s.getpopulationsize(); i++) {// 60 30 //200
	 * 
	 * newSolution = new Solution(problem_);
	 * 
	 * // writer.println(newSolution.getoperations()); //
	 * writer.println("zzzahraaaaaaaa"); NSGAII.counter=NSGAII.counter+1;
	 ...
	 * Generations // while (checkiteration==false) { while (evaluations <
	 * maxEvaluations) { System.out.println("startNSGAII"); // Create the offSpring
	 * solutionSet offspringPopulation = new SolutionSet(populationSize); Solution[]
	 * parents = new Solution[2]; for (int i = 0; i < (populationSize / 2); i++) {
	 * if (evaluations < maxEvaluations) { //obtain parents parents[0] = (Solution)
	 * selectionOperator.execute(population); parents[1] = (Solution)
	 * selectionOperator.execute(population); System.out.println(
	 * parents[0].getoperations());
	 * writer.println("choosefrompopulation-firstparent");
	 * writer.println(parents[0].getoperations());
	 * writer.println(parents[0].getindex()); //
	 * System.out.println("indexcrossover"); // System.out.println(
	 * parents[0].getindex()); // System.out.println( parents[1].getindex());
	 ...
	 * System.out.println("offspring"); writer.println("aftercrossover"); //
	 * System.out.println("aftercrossover"); // System.out.println(
	 * offSpring[0].getoperations()); // System.out.println(
	 * offSpring[1].getoperations()); // System.out.println( offSpring[0].inorout);
	 ...
	 * System.out.println("mutationafter"); writer.println("aftermutation"); //
	 * System.out.println( offSpring[0].getoperations());
	 * writer.println(offSpring[0].getoperations());
	 * mutationOperator.execute(offSpring[1]); /* System.out.println(
	 * offSpring[1].getoperations()); System.out.println( offSpring[0].inorout);
	 * System.out.println( offSpring[1].inorout); System.out.println(
	 * offSpring[0].classifiersolution); System.out.println(
	 * offSpring[1].classifiersolution);
	 */
	/*
	 * NSGAII.counter=NSGAII.counter+1;
	 * writer.println(offSpring[1].getoperations());
	 * writer.println(offSpring[0].inorout); writer.println(offSpring[1].inorout);
	 * NSGAII.statemutcrossinitial="mutation"; Class2Rel.checkoffspring=1;
	 * 
	 * problem_.evaluate(offSpring[0],this.csvWriterpareto);
	 * problem_.evaluateConstraints(offSpring[0]);
	 * writer.println("offSpring[0].getoperations()");
	 * writer.println(offSpring[0].getindex()); NSGAII.counter=NSGAII.counter+1;
	 * Class2Rel.checkoffspring=1;
	 * 
	 * problem_.evaluate(offSpring[1],this.csvWriterpareto);
	 * problem_.evaluateConstraints(offSpring[1]);
	 * writer.println("offSpring[1].getoperations()");
	 * writer.println(offSpring[1].getindex()); //bod
	 * System.out.println("evaluation"); //bod System.out.println(evaluations);
	 * offspringPopulation.add(offSpring[0]); offspringPopulation.add(offSpring[1]);
	 * 
	 * evaluations += 2; } // if } // for //
	 * System.out.println("offspringsizeeeee"); //
	 * System.out.println(offspringPopulation.size()); // Create the solutionSet
	 * union of solutionSet and offSpring union = ((SolutionSet)
	 * population).union(offspringPopulation);
	 * 
	 * // Ranking the union Ranking ranking = new
	 * Ranking(union,this.csvWriterpareto);
	 * 
	 * int remain = populationSize; int index = 0; SolutionSet front = null;
	 * population.clear();
	 * 
	 * // Obtain the next front front = ranking.getSubfront(index);
	 * writer.println("ranking"); writer.println(front.size()); for (int k = 0; k <
	 * front.size(); k++) { writer.println(front.get(k));
	 * writer.println(front.get(k).getindex());
	 * writer.println(front.get(k).getnumbererror());
	 * if(front.get(k).getnumbererror()==0) checkiteration=true; }
	 * 
	 * while ((remain > 0) && (remain >= front.size())) { //Assign crowding distance
	 * to individuals distance.crowdingDistanceAssignment(front,
	 * problem_.getNumberOfObjectives()); //Add the individuals of this front for
	 * (int k = 0; k < front.size(); k++) { population.add(front.get(k)); } // for
	 * 
	 * //Decrement remain remain = remain - front.size();
	 * 
	 * //Obtain the next front index++; if (remain > 0) { front =
	 * ranking.getSubfront(index); } // if } // while
	 * 
	 * // Remain is less than front(index).size, insert only the best one if (remain
	 * > 0) { // front contains individuals to insert
	 * distance.crowdingDistanceAssignment(front, problem_.getNumberOfObjectives());
	 * front.sort(new CrowdingComparator()); for (int k = 0; k < remain; k++) {
	 * population.add(front.get(k)); } // for
	 * 
	 * remain = 0; } // if
	 * 
	 * // This piece of code shows how to use the indicator object into the code //
	 * of NSGA-II. In particular, it finds the number of evaluations required // by
	 * the algorithm to obtain a Pareto front with a hypervolume higher // than the
	 * hypervolume of the true Pareto front. if ((indicators != null) &&
	 * (requiredEvaluations == 0)) { double HV =
	 * indicators.getHypervolume(population); if (HV >= (0.98 *
	 * indicators.getTrueParetoFrontHypervolume())) { requiredEvaluations =
	 * evaluations; } // if } // if // System.out.println("after crossover");
	 ...
	 * 
	 * // Return the first non-dominated front Ranking ranking = new
	 * Ranking(population,this.csvWriterpareto);
	 * ranking.getSubfront(0).printFeasibleFUN("FUN_NSGAII") ;
	 * writer.println("subfront"); writer.println(ranking.getSubfront(0).size());
	 * writer.close(); try { this.csvWriterpareto.flush();
	 * this.csvWriterpareto.close(); } catch (IOException e) { // TODO
	 * Auto-generated catch block e.printStackTrace(); }
	 * 
	 * // writer2.close(); return ranking.getSubfront(0); } // execute
	 * 
	 */
	...
	private void FindFaultyRule() {
		...
		if (NSGAII.startsituation == false) {
			...
			if (main.postprocessing == false) {
				...
				try {
					bf = new BufferedReader(new FileReader(NSGAII.inputfaultytransformation));
				} catch (FileNotFoundException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
